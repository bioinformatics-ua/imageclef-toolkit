...
class GlobalParams(NamedTuple):
    ...
    data_format: Any
    num_classes: int
    width_coefficient: Any
    depth_coefficient: Any
    depth_divisor: Any
    min_depth: int
    drop_connect_rate: Any
    relu_fn: Any
    batch_norm: Any
    use_se: bool
...
class Model(tf.keras.Model):
  ...
  def __init__(self, blocks_args: List[BlockArgs] = None, global_params: GlobalParams = None, last_fn = None):
  ...
  def call(self, inputs, training=True, features_only=None):
    ...
    if isinstance(inputs, list) or isinstance(inputs, tuple):
      # encoder only uses one of the inputs
      inputs = inputs[0]
    outputs = None
    self.endpoints = {}
    # this hack is required because TensorFlow is zt00pid
    setattr(self.endpoints, '_unconditional_dependency_names', {})
